    ...
    <>
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 my-1">
        <div className="stat-box relative">
          ...
          <div className="title">
            <strong className="text-black">Full Professors(FP)</strong>
          </div>
          <h1 className="total mt-2">
            <FontAwesomeIcon className="text-secondary" icon={faAreaChart} />{' '}
            {isLoading ? <Wait /> : statistics?.percentageFullProfessors.toFixed(1) + '%'}
          </h1>
          <em className="absolute bottom-0 right-5">
            <strong className="text-green-600 small">{isLoading ? <Wait /> : statistics?.percentageFullProfessors.toFixed(1) + '%'}</strong> Full Professors
          </em>
        </div>
      </div>
      {/*  */}
