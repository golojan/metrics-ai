...
import { FacultiesInfo } from '../../libs/interfaces';
import Link from 'next/link';
import { noAction } from '../../libs/utils';

const tableIcons: Icons<FacultiesInfo> = {
...
const AuthFaculties = (props: Props) => {
  const { title, data, loading } = props;
  const [facultyId, setFacultyId] = useState<string>('');

  const columns: Column<FacultiesInfo>[] = [
    {
      title: 'Faculty',
      field: 'facultyName',
    },
    ...
    },
    {
      ...
      render: (rowData) => (
        <Link
          href={`/dashboard/reports/faculties/${rowData._id}`}
  ...
  const options = {
    ...
    rowStyle: (rowData) => ({
      backgroundColor: facultyId === rowData._id ? '#EEE' : '#FFF',
    }),
  };
  ...
  return (
    <MaterialTable
      ...
      columns={columns}
      icons={tableIcons}
      onRowClick={(evt, row) => setFacultyId(row._id)}
    />
  );
};

export default AuthFaculties;
